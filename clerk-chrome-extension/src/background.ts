...
if (!publishableKey) {
  throw new Error(
    "Please add PLASMO_PUBLIC_CLERK_PUBLISHABLE_KEY to your .env file"
  )
}
...
const hasClerkSyncCookies = async (): Promise<boolean> => {
  try {
    ...
    return cookies.some(
      (c) => c.name.includes("__clerk") || c.name.includes("__client")
    )
  } catch {
    return false
  }
...
const invalidateClerkClient = () => {
  debug.any(
    ["auth", "clerk", "background"],
    "Invalidating Clerk client singleton"
  )
    ...
    try {
      ...
      if (!clerkClient.session) {
        ...
        if (hasCookies) {
          debug.any(
            ["auth", "clerk", "background"],
            "refreshClerkClient: recreating stale client",
            { reason }
          )
      ...
      debug.any(["auth", "clerk", "background"], "Clerk refreshed", {
        reason,
...
chrome.tabs.onUpdated.addListener((tabId, changeInfo, tab) => {
  ...
  if (isTargetSite(url)) {
    debug.any(["interceptor", "background"], "Injecting interceptor into tab", {
      tabId,
      url
    })
...
chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
  ...
  if (message.action === "openAuthTab") {
    chrome.tabs.create({
      url: "https://momentum.ubi.studio/extension/sign-in?source=extension"
    })
    sendResponse({ success: true })
    return true
  }
    ...
    ;(async () => {
      try {
        ...
        if (!hasSessionAfter) {
          ...
          debug.any(
            ["auth", "clerk", "background"],
            "refreshClerkAuth: no session, checking cookies",
            { hasCookies }
          )
          ...
          if (hasCookies) {
            debug.any(
              ["auth", "clerk", "background"],
              "refreshClerkAuth: cookies present, recreating client"
            )
            invalidateClerkClient()
            clerkClient = await getClerkClient()
            await clerkClient.load({ standardBrowser: false })
            ...
            debug.any(
              ["auth", "clerk", "background"],
              "refreshClerkAuth: after recreate",
              { hasSessionAfter }
            )
          }
        }
  ...
  if (message.action === "getClerkToken") {
    getClerkClient()
      .then(async (clerkClient) => {
        ...
        debug.any(["auth", "clerk", "token"], "getClerkToken: entry", {
          hasSessionInitially
        })
        ...
        if (!clerkClient.session) {
          debug.any(
            ["auth", "clerk", "token"],
            "getClerkToken: invoking refresh"
          )
          try {
        ...
        if (!clerkClient.session) {
          debug.any(
            ["auth", "clerk", "token"],
            "getClerkToken: no session after refresh"
          )
          sendResponse({
            success: false,
            token: null,
